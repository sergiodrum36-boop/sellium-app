...
var _check_parserSellOutClientes = require("./_check_parserSellOutClientes");
function _interopRequireWildcard(e, t) { if ("function" == typeof WeakMap) var r = new WeakMap(), n = new WeakMap(); return (_interopRequireWildcard = function (e, t) { if (!t && e && e.__esModule) return e; var o, i, f = { __proto__: null, default: e }; if (null === e || "object" != typeof e && "function" != typeof e) return f; if (o = t ? n : r) { if (o.has(e)) return o.get(e); o.set(e, f); } for (const t in e) "default" !== t && {}.hasOwnProperty.call(e, t) && ((i = (o = Object.defineProperty) && Object.getOwnPropertyDescriptor(e, t)) && (i.get || i.set) ? o(f, t, i) : f[t] = e[t]); return f; })(e, t); }
function workbookDeUnaHoja(filas) {
  let nombreHoja = arguments.length > 1 && arguments[1] !== undefined ? arguments[1] : 'Sheet1';
...
describe('parseSellOutClientes: caso básico con Ventas/Promo/Regalos', () => {
  const wb = workbookDeUnaHoja([['CLIENTE', 'COD. CLIENTE', 'NIF', 'TIPOLOGIA', 'C.CIAL', 'PREVENTISTA', 'FECHA', 'PRODUCTO', 'VENTAS', 'PROMO', 'REGALOS', 'NETO'], ['Bar Pepe', 'C001', 'B12345678', 'Hosteleria', '1', 'Luis', '15/03/2026', 'Rioja Crianza', 10, 2, 0, '120,50'], ['Restaurante X', 'C002', '', 'Restauracion', '2', 'Ana', '2026-03-20', 'Palomo Cojo', 5, 0, 1, 60]]);
  const resultado = (0, _check_parserSellOutClientes.parseSellOutClientes)(wb);
...
describe('parseSellOutClientes: solo columna de unidades totales (sin desglose)', () => {
  const wb = workbookDeUnaHoja([['CLIENTE', 'ARTICULO', 'UNIDADES', 'IMPORTE'], ['Covirán Las Americas', 'Albariño', 8, 95.2]]);
  const resultado = (0, _check_parserSellOutClientes.parseSellOutClientes)(wb);
...
describe('parseSellOutClientes: "CLIENTE - COMERCIO" sustituye al nombre mostrado', () => {
  const wb = workbookDeUnaHoja([['CLIENTE', 'CLIENTE - COMERCIO', 'ARTICULO', 'TOTALES'], ['EXPLOTACIONES GARAJAO S.L.', 'COVIRAN LAS AMERICAS', 'Rioja Crianza', 4], ['DISTRIBUCIONES SUR S.L.', '', 'Palomo Cojo', 3]]);
  const resultado = (0, _check_parserSellOutClientes.parseSellOutClientes)(wb);
...
describe('parseSellOutClientes: filas a excluir', () => {
  const wb = workbookDeUnaHoja([['CLIENTE', 'ARTICULO', 'TOTALES'], ['Bar Pepe', 'Rioja Crianza', 5], ['', '', ''], ['TOTAL GENERAL', 'Rioja Crianza', 100], ['Restaurante X', '', 3], ['', 'Palomo Cojo', 2]]);
  const resultado = (0, _check_parserSellOutClientes.parseSellOutClientes)(wb);
...
describe('parseSellOutClientes: selección de hoja (varias hojas, gana la de más detalle)', () => {
  const wb = XLSX.utils.book_new();
  const wsResumen = XLSX.utils.aoa_to_sheet([['CLIENTE', 'TOTALES'], ['Total General', 999]]);
  const wsDetalle = XLSX.utils.aoa_to_sheet([['CLIENTE', 'ARTICULO', 'TOTALES'], ['Bar Pepe', 'Rioja Crianza', 5], ['Restaurante X', 'Palomo Cojo', 3], ['Hotel Playa', 'Albariño', 7]]);
...
describe('parseSellOutClientes: avisos cuando faltan columnas relevantes', () => {
  test('sin columna de código de cliente, avisa (aunque igualmente extrae las filas)', () => {
    const wb = workbookDeUnaHoja([['CLIENTE', 'ARTICULO', 'TOTALES'], ['Bar Pepe', 'Rioja Crianza', 5]]);
    const resultado = (0, _check_parserSellOutClientes.parseSellOutClientes)(wb);
    expect(resultado.filas).toHaveLength(1);
  ...
  test('sin ninguna columna de unidades, avisa explícitamente', () => {
    const wb = workbookDeUnaHoja([['CLIENTE', 'ARTICULO', 'NIF'], ['Bar Pepe', 'Rioja Crianza', 'B123']]);
    const resultado = (0, _check_parserSellOutClientes.parseSellOutClientes)(wb);
  ...
  test('ningún archivo reconocible: sin hoja con Cliente y Producto', () => {
    const wb = workbookDeUnaHoja([['COLUMNA_RARA_1', 'COLUMNA_RARA_2'], ['x', 'y']]);
    const resultado = (0, _check_parserSellOutClientes.parseSellOutClientes)(wb);
...
describe('parseSellOutClientes: columna "distribuidor" (archivos que mezclan varios)', () => {
  test('cuando existe la columna EMPRESA/DISTRIBUIDOR, se guarda por fila', () => {
    const wb = workbookDeUnaHoja([['EMPRESA', 'CLIENTE', 'ARTICULO', 'TOTALES'], ['MANUEL VEGA S.L.', 'Bar Pepe', 'Rioja Crianza', 5], ['VEGA Y GIJON S.L.', 'Restaurante X', 'Palomo Cojo', 3]]);
    const resultado = (0, _check_parserSellOutClientes.parseSellOutClientes)(wb);
