...
import { Plus, Trash2, Handshake, Pencil, X, Eye, Download, Save } from 'lucide-react';
import {
...
import { calcularConsumoAcuerdo, calcularEstadoVigencia, diasHastaFin, calcularSiguienteNumeroPropuesta } from './seguimientoAcuerdos';
...
// Tipos de negocio de hostelería/hotelería (27/07/2026, a petición de
// Sergio: "el desplegable de tipología debería salir elegible, escoge los
// tipos de negocios de hotelería y hostelería") — antes era texto libre.
// 'OTRO' revela un campo de texto para no bloquear un caso que no esté en
// esta lista (ver FormularioAcuerdo).
const TIPOS_NEGOCIO = [
  { value: 'RTE', label: 'Restaurante (RTE)' },
  { value: 'BAR', label: 'Bar' },
  { value: 'CHIRINGUITO', label: 'Chiringuito' },
  { value: 'HOTEL', label: 'Hotel' },
  { value: 'CAFETERIA', label: 'Cafetería' },
  { value: 'DISCOTECA_PUB', label: 'Discoteca / Pub' },
...
// `tarifaPorMarca` (Map id_marca -> pvp_iva, 27/07/2026): al elegir una marca
// con precio ya guardado se autorrellena el PVP+IVA (solo si el campo estaba
// vacío, para no pisar un valor que ya se hubiera escrito a mano). El botón
// de guardar (icono disco) guarda el PVP+IVA actual de esa fila como el
// nuevo valor por defecto de esa marca — "puede haber variaciones, pero con
// la opción de poder cambiarlo" (palabras de Sergio).
function EditorReferencias({ referencias, onChange, marcasGlobales, tarifaPorMarca, onGuardarTarifa }) {
  ...
  const cambiarMarca = (i, idMarca) => {
    ...
    const tarifa = tarifaPorMarca.get(idMarca);
    const copia = referencias.map((r, idx) => {
      if (idx !== i) return r;
      const siguiente = { ...r, id_marca: idMarca, nombre_marca: marca?.nombre_marca || '' };
      if (!r.pvp_iva && tarifa !== undefined) siguiente.pvp_iva = tarifa;
      return siguiente;
    });
    onChange(copia);
  };
            ...
            <tr key={i}>
              ...
              <td className={tdClasses}>
                <div className="flex items-center gap-1">
                  ...
                  <button
                    type="button"
                    disabled={!r.id_marca || r.pvp_iva === ''}
                    onClick={() => onGuardarTarifa(r.id_marca, r.nombre_marca, r.pvp_iva)}
                    className="text-slate-400 hover:text-indigo-600 dark:hover:text-indigo-400 disabled:opacity-30 disabled:cursor-not-allowed"
                    title="Guardar como tarifa por defecto de esta marca"
                  >
                    <Save size={14} />
                  </button>
                </div>
...
function FormularioAcuerdo({ valores, onChange, distribuidores, clientesSellOut, marcasGlobales, tarifaPorMarca, onGuardarTarifa, numeroEditable }) {
  ...
  // Tipo de negocio: 'OTRO' (o cualquier valor que ya no esté en la lista
  // fija, ej. datos antiguos con texto libre) revela el campo de texto.
  const tipoCoincideConLista = TIPOS_NEGOCIO.some((t) => t.value === valores.tipo_negocio && t.value !== 'OTRO');
    ...
    <div className="space-y-5">
      <div>
        ...
        <div className="grid grid-cols-2 md:grid-cols-3 gap-3">
          <div>
            <label className={`${etiqueta} block mb-1`}>Nº propuesta {numeroEditable ? '' : <span className="font-normal normal-case text-slate-400">(automático)</span>}</label>
            <input type="text" value={valores.numero} readOnly={!numeroEditable} onChange={(e) => set('numero', e.target.value)} className={`${inputClasses} w-full ${numeroEditable ? '' : 'opacity-70 cursor-not-allowed'}`} />
          </div>
          <div>
          ...
          <div>
            ...
            <select
              value={tipoCoincideConLista ? valores.tipo_negocio : 'OTRO'}
              onChange={(e) => set('tipo_negocio', e.target.value === 'OTRO' ? '' : e.target.value)}
              className={`${inputClasses} w-full`}
            >
              {TIPOS_NEGOCIO.map((t) => <option key={t.value} value={t.value}>{t.label}</option>)}
            </select>
            {!tipoCoincideConLista && (
              <input
                type="text" value={valores.tipo_negocio} onChange={(e) => set('tipo_negocio', e.target.value)}
                className={`${inputClasses} w-full mt-1.5`} placeholder="Especificar tipo de negocio"
              />
            )}
          </div>
        </div>
          ...
          <div className="flex flex-wrap items-center gap-3">
            {!valores.clienteEsNuevo ? (
              <div className="w-full max-w-sm">
                <SelectorBuscable
                  valor={valores.id_cliente}
                  onSeleccionar={(id) => cambiarCliente(id || '')}
                  opciones={clientesDelDistribuidor}
                  getId={(c) => c.id}
                  getLabel={(c) => c.nombre_cliente}
                  placeholder={valores.id_distribuidor ? 'Buscar cliente...' : 'Elige antes un distribuidor'}
                  disabled={!valores.id_distribuidor}
                  vacioTexto="Ningún cliente de este distribuidor coincide"
                />
              </div>
            ) : (
      ...
      <div>
        ...
        <EditorReferencias
          ...
          onChange={(referencias) => set('referencias', referencias)}
          marcasGlobales={marcasGlobales}
          tarifaPorMarca={tarifaPorMarca}
          onGuardarTarifa={onGuardarTarifa}
        />
      </div>
...
function PantallaAcuerdosClientes({ idUsuario, bloqueadoPorTodos = false }) {
  ...
  const [tarifaMarcas, setTarifaMarcas] = useState([]);
  ...
  const cargarTodo = useCallback(async () => {
    if (!idUsuario) {
      setDistribuidores([]); setMarcasGlobales([]); setClientesSellOut([]); setMovimientos([]); setAcuerdos([]); setTarifaMarcas([]);
      setCargando(false);
      return;
    }
    setCargando(true);
    try {
      const [dist, marcas, clientes, movs, ac, tarifas] = await Promise.all([
  ...
  const tarifaPorMarca = useMemo(() => new Map(tarifaMarcas.map((t) => [t.id_marca, t.pvp_iva])), [tarifaMarcas]);

  // Recarga solo la tarifa (no todo cargarTodo, que también relee el
  // histórico completo de movimientos) tras guardar un precio nuevo.
  const handleGuardarTarifa = async (idMarca, nombreMarca, pvpIva) => {
    try {
      await guardarTarifaMarca(idMarca, nombreMarca, pvpIva);
      setTarifaMarcas(await getTarifaMarcas());
    } catch (error) {
      console.error('Error guardando la tarifa de la marca:', error);
      alert('Error al guardar la tarifa: ' + error.message);
    }
  };

  // Número de propuesta automático (27/07/2026, a petición de Sergio): solo
  // en modo 'crear' — al editar un acuerdo ya guardado se respeta su número
  // original. El año sale de "Fecha de la propuesta" si ya se ha rellenado,
  // si no del año actual; se recalcula si el usuario cambia esa fecha.
  useEffect(() => {
    if (modo !== 'crear') return;
    const anio = formulario.fecha_propuesta ? Number(formulario.fecha_propuesta.slice(0, 4)) : new Date().getFullYear();
    const siguiente = calcularSiguienteNumeroPropuesta(acuerdos, anio);
    setFormulario((prev) => (prev.numero === siguiente ? prev : { ...prev, numero: siguiente }));
    // eslint-disable-next-line react-hooks/exhaustive-deps
  }, [modo, formulario.fecha_propuesta, acuerdos]);
        ...
        <div className="fixed inset-0 z-50 bg-black/40 flex items-center justify-center p-4">
          <div className="bg-white dark:bg-slate-800 rounded-xl p-5 max-w-4xl w-full max-h-[90vh] overflow-y-auto">
            ...
            <FormularioAcuerdo
              ...
              clientesSellOut={clientesSellOut}
              marcasGlobales={marcasGlobales}
              tarifaPorMarca={tarifaPorMarca}
              onGuardarTarifa={handleGuardarTarifa}
              numeroEditable={modo === 'editar'}
            />
