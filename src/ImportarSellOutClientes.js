 ...
 *
 * Memoria de reconciliación de Productos (a petición de Sergio, 2026-07-18):
 * al reimportar un periodo nuevo de un distribuidor ya existente, cada
 * decisión producto->marca que se confirma se guarda como "alias" (colección
 * `aliasProductosSellOut`, ver firebaseApi.js). La SIGUIENTE importación de
 * ESE MISMO distribuidor reconoce el texto exacto del producto tal como
 * viene en su archivo y preselecciona automáticamente la misma marca, sin
 * depender de que el nombre coincida ni remotamente con el catálogo global
 * de Marcas (ver preseleccionarMarcas más abajo — el alias tiene prioridad
 * sobre el match por nombre exacto y sobre el parecido difuso).
 */
...
function ImportarSellOutClientes({ idUsuario, listaDistribuidores, marcasGlobales, onImportComplete }) {
  ...
  const [aliasProductosExistentes, setAliasProductosExistentes] = useState([]); // memoria producto->marca de importaciones anteriores de este distribuidor
  ...
  const cargarDatosDistribuidor = async (idDistribuidor) => {
    ...
    setAliasProductosExistentes([]);
    setDecisionCliente({});
    setDecisionConflicto({});
    if (!idDistribuidor) return;
    setCargandoDatosDistribuidor(true);
    try {
      const [clientes, movimientos, aliasProductos] = await Promise.all([
        ...
        getAliasProductosSellOutPorDistribuidor(idUsuario, idDistribuidor)
      ]);
      setClientesExistentes(clientes);
      setMesesExistentes(new Set(movimientos.map(m => m.mes_ano).filter(Boolean)));
      setAliasProductosExistentes(aliasProductos);
      preseleccionarClientes(clientes);
      preseleccionarMarcas(aliasProductos);
    } catch (err) {
      console.error(err);
  ...
  // Preselecciona la decisión de cada Producto (=Marca), con esta prioridad:
  //   1º Alias guardado en una importación anterior de ESTE distribuidor
  //      (el texto del producto coincide EXACTO con uno ya decidido antes —
  //      la señal más fiable posible, es la propia decisión previa del
  //      usuario para este mismo distribuidor).
  //   2º Nombre exacto contra el catálogo global de Marcas.
  //   3º Parecido difuso (>=85%) contra el catálogo global de Marcas.
  //   4º Crear como marca nueva.
  const preseleccionarMarcas = (aliasProductos) => {
    if (!resultado) return;
    const mapaAlias = new Map((aliasProductos || []).map(a => [a.producto_normalizado, a]));
    const productos = [...new Set(resultado.filas.map(f => f.producto))];

    setDecisionMarca(prev => {
      const nueva = { ...prev };
      productos.forEach(producto => {
        const alias = mapaAlias.get(norm(producto));
        if (alias) {
          nueva[producto] = { accion: 'usar_existente', idExistente: alias.id_marca };
          return;
        }
        // Si no hay alias, se respeta la preselección que ya se hizo al leer
        // el archivo (nombre exacto / parecido difuso / crear nueva).
        if (!nueva[producto]) nueva[producto] = { accion: 'crear', idExistente: null };
      });
      return nueva;
    });
  };
  ...
  // Cuántos de los productos de este archivo se han reconocido por alias de
  // una importación anterior de este mismo distribuidor (solo informativo).
  const mapaAliasProductos = new Map((aliasProductosExistentes || []).map(a => [a.producto_normalizado, a]));
  const productosReconocidosPorAlias = productosExcel.filter(p => mapaAliasProductos.has(norm(p))).length;
  ...
  const handleConfirmarImportacion = async () => {
    ...
    try {
      ...
      // --- c-bis) Guardar alias producto->marca de esta importación, para
      // que la SIGUIENTE de este mismo distribuidor los reconozca solos ---
      const entradasAlias = productosExcel
        .filter(p => idPorProducto[p])
        .map(p => ({ producto_normalizado: norm(p), id_marca: idPorProducto[p], nombre_marca: nombrePorProducto[p] || p }));
      if (entradasAlias.length > 0) {
        await saveAliasProductosSellOut(idUsuario, idDistribuidor, entradasAlias);
      }
      ...
      const filasFinal = filasFiltradas.filter(filaValida).map(f => {
        ...
        return {
          ...
          facturacion_euros: f.facturacion
        };
      });
            ...
            <>
              ...
              <div className={`${tarjeta} mb-4`}>
                ...
                <div className="rounded-lg border border-slate-200 dark:border-slate-700">
                  <TablaOrdenable
                    claseContenedor="overflow-x-auto max-h-96 overflow-y-auto"
                    filas={clientesExcelList}
                    keyExtractor={([clave]) => clave}
                    columnas={[
                      { titulo: 'Cliente (archivo)', valor: ([, filaRep]) => filaRep.cliente || '', render: ([, filaRep]) => filaRep.cliente },
                      { titulo: 'Cod./NIF', valor: ([, filaRep]) => filaRep.cod_cliente || filaRep.nif || '', render: ([, filaRep]) => filaRep.cod_cliente || filaRep.nif || '—' },
                      {
                        titulo: 'Filas', derecha: true,
                        valor: ([clave]) => filasFiltradas.filter(f => claveClienteExcel(f) === clave).length,
                        render: ([clave]) => filasFiltradas.filter(f => claveClienteExcel(f) === clave).length,
                      },
                      {
                        titulo: 'Decisión', render: ([clave, filaRep]) => {
              ...
              <div className={`${tarjeta} mb-4`}>
                ...
                {productosReconocidosPorAlias > 0 && (
                  <p className="text-xs text-emerald-600 dark:text-emerald-400 mb-3">
                    {productosReconocidosPorAlias} de estos productos se han reconocido automáticamente de una importación anterior de este distribuidor.
                  </p>
                )}
                <div className="rounded-lg border border-slate-200 dark:border-slate-700">
                  <TablaOrdenable
                    claseContenedor="overflow-x-auto max-h-96 overflow-y-auto"
                    filas={productosExcel}
                    keyExtractor={producto => producto}
                    columnas={[
                      { titulo: 'Producto (archivo)', valor: producto => producto, render: producto => producto },
                      {
                        titulo: 'Filas', derecha: true,
                        valor: producto => filasFiltradas.filter(f => f.producto === producto).length,
                        render: producto => filasFiltradas.filter(f => f.producto === producto).length,
                      },
                      {
                        titulo: 'Decisión', render: producto => {
