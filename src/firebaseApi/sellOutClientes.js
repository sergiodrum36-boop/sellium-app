...
// --- 21a-bis. GEOLOCALIZACIÓN DE UN CLIENTE FINAL (26/07/2026, a petición
// de Sergio: "visión de mancha de aceite con clientes por zonas") ---
// Edición IN-PLACE (updateDoc, NO borrar+crear como el resto de esta app):
// `movimientosSellOutClientes.id_cliente` referencia este documento por id,
// así que recrearlo dejaría huérfanas todas sus líneas de venta ya
// importadas — ver el aviso junto a esta colección en firestore.rules
// (esCambioGeolocalizacionCliente, acota la escritura a exactamente estos 4
// campos). `cambios` = un subconjunto de { direccion, latitud, longitud,
// id_zona }.
export const actualizarClienteSellOut = async (idUsuario, idDistribuidor, docId, cambios) => {
  const ref = doc(db, "clientesSellOut", docId);
