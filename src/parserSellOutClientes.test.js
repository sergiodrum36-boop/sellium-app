/*
 * parserSellOutClientes.test.js
 * Tests del parser de Sell-Out por Cliente Final. Solo se exporta
 * parseSellOutClientes(workbook), así que los tests construyen workbooks XLSX
 * sintéticos reales (con la propia librería 'xlsx', ya dependencia del
 * proyecto) y comprueban el resultado público — ejercita la detección de
 * columnas, la heurística de selección de hoja y el cálculo de totales tal
 * como los usa de verdad ImportarSellOutClientes.js, sin tener que exportar
 * las funciones internas solo para testearlas sueltas.
 */
import * as XLSX from 'xlsx';
import { parseSellOutClientes } from './parserSellOutClientes';

// Construye un workbook con una sola hoja a partir de una matriz de filas
// (array de arrays, primera fila = cabecera).
...
describe('parseSellOutClientes: caso básico con Ventas/Promo/Regalos', () => {
  ...
  test('totales se calcula como ventas+promo+regalos si no hay columna propia', () => {
    expect(resultado.filas[0].totales).toBe(12); // 10+2+0
    expect(resultado.filas[1].totales).toBe(6); // 5+0+1
  });
...
describe('parseSellOutClientes: "CLIENTE - COMERCIO" sustituye al nombre mostrado', () => {
  const wb = workbookDeUnaHoja([
    ...
    ['DISTRIBUCIONES SUR S.L.', '', 'Palomo Cojo', 3] // sin nombre comercial: usa la razón social
  ]);
  const resultado = parseSellOutClientes(wb);
...
describe('parseSellOutClientes: filas a excluir', () => {
  const wb = workbookDeUnaHoja([
    ...
    ['', '', ''], // fila en blanco
    ['TOTAL GENERAL', 'Rioja Crianza', 100], // fila de subtotal
    ['Restaurante X', '', 3], // sin producto: no es una fila de detalle válida
    ['', 'Palomo Cojo', 2] // sin cliente: tampoco
  ]);
  const resultado = parseSellOutClientes(wb);
...
describe('parseSellOutClientes: selección de hoja (varias hojas, gana la de más detalle)', () => {
  const wb = XLSX.utils.book_new();
  // "Resumen": una tabla dinámica de apoyo, con menos filas de detalle real.
  const wsResumen = XLSX.utils.aoa_to_sheet([
    ['CLIENTE', 'TOTALES'],
    ['Total General', 999]
  ]);
  // "Sheet1": el detalle de verdad, con más filas reconocidas.
